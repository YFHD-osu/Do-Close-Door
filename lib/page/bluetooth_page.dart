    ...
    switch(command[1]){
      ...
      case "stop_record":
        if (cameraController.value.isRecordingVideo) {
          ...
          print("[Saving] ${appPath!.path}/${now.year}-${now.month}-${now.day}-${now.hour}-${now.minute}-${now.second}-$lastTemperature-${command[2]}.mp4");
          media.saveTo("${appPath!.path}/${now.year}-${now.month}-${now.day}-${now.hour}-${now.minute}-${now.second}-$lastTemperature-${command[2]}.mp4");
